...
No_training_samples_per_User = 3
...
for i in range(1, No_of_Users+1):
   ...
   for k in range(0,Signature_Samples_Per_User): 
           ...
           if (k not in Random_Selected_List2):
               Final_test_List2.append(Current_writer_forgery_image_list[k])
#####################################################################################################################
...
t = time.time()
hist = model.fit(X_train,Y_train,batch_size=64,epochs=epoch, verbose=1,validation_data=(X_test, Y_test),)
