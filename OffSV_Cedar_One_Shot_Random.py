...
for i in range(1, No_of_Users+1):
   ...
   for j in range(0,No_training_samples_per_User): # Training
       
       Final_train_List1.append(Current_writer_genuine_image_list[Random_Selected_List1[j]])
   
   for k in range(0,Signature_Samples_Per_User): 
           
           if (k not in Random_Selected_List1):
               Final_test_List1.append(Current_writer_genuine_image_list[k])
       
   for j in  range(1, No_of_Users+1): # Testing
       if i != j:  
          Not_Current_writer_genuine_image_list, _ = Get_Full_Data(j) 
          Random_Selected_List3 = random.sample(range(0, 23), 2) # rand int considers last value also. List starts from 0. hence, 0 to 23
          Final_train_List2.append(Not_Current_writer_genuine_image_list[Random_Selected_List3[0]])
          Final_test_List2.append(Not_Current_writer_genuine_image_list[Random_Selected_List3[1]])
#####################################################################################################################
...
No_Forgery_train_Labels = No_of_Users * (No_of_Users-1)


No_Genuine_test_Labels =  No_of_Users * No_testing_samples_per_User
No_Forgery_test_Labels =  No_of_Users * (No_of_Users-1)
...
t = time.time()
hist = model.fit(X_train,Y_train,batch_size=8,epochs=epoch, verbose=1,validation_data=(X_test, Y_test),)
